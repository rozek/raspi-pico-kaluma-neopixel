...
/**** normalized - non-linear mapping of 0...255 to 0...255 ****/

  let normalized = new Uint8Array(256);
  for (let i = 0; i < 256; i++) {
    normalized[i] = (
      i === 0
      ? 0
      : Math.max(0,Math.min(Math.round(1 + 256/4*(i/256)*(i/256)),255))
    );
  }
...
let Display = SPIDisplay(null, 16,16);
  ...
  for (let x = 0; x < 16; x++) {
    for (let y = 0; y < 16; y++) {
      let RGB = Display.HSLtoRGB(x/16,1,y/32);
        let R = normalized[RGB[0]];
        let G = normalized[RGB[1]];
        let B = normalized[RGB[2]];
      Display.setPixelRGB(x,y, R,G,B);
    }
  }
Display.show();
